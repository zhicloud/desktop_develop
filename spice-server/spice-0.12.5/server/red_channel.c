...
static void red_channel_client_send_item(RedChannelClient *rcc, PipeItem *item)
{
    ...
    if (!handled) {
        //spice_info("send_item =======   0x%x \n ",rcc->channel->channel_cbs.send_item);
...
void red_channel_client_pipe_clear(RedChannelClient *rcc)
{
    ...
    while ((item = (PipeItem *)ring_get_head(&rcc->pipe))) {
        ring_remove(&item->link);
        spice_debug("red_channel_client_pipe_clear");
        red_channel_client_release_item(rcc, item, FALSE);
    }
    rcc->pipe_size = 0;
}
                                                ...
                                                PipeItem *item)
{
    ...
    spice_debug("red_channel_client_pipe_remove_and_release");
    red_channel_client_release_item(rcc, item, FALSE);
}
