...
//add by lcx for h264
void red_dispatcher_on_svc_change(void)
{
    RedWorkerMessageSetStreamingVideoCompression payload;
    RedDispatcher *now = dispatchers;
    spice_info("i come to red_dispatcher_on_svc_change");
    while (now){
        payload.streaming_video_compression = streaming_video_compression;
        dispatcher_send_message(&now->dispatcher,
