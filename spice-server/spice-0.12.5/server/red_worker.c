...
#define RED_STREAM_FULL_H264_CODE_THREADING
...
#define RED_GLOBAL_H264_STREAM_ID (NUM_STREAMS + 100) 
...
enum {
    ...
    PIPE_ITEM_TYPE_STREAM_THREADING, 
#endif
};
...
typedef struct TimOutItem {
    PipeItem link;
    int refs;
}TimOutItem;

typedef struct H264ThreadingItem {
    PipeItem link;
    int refs;
}H264ThreadingItem;
...
typedef struct RedWorker {
    ...
    //add by lcx for init ffmpeg
    int more_video_compress;
...
/*
    ...
    */
    ...
    RedEncodingThreading * threading;
#endif
    //add by lcx 
    int stream_video_h264_all_fps;
    int stream_video_h264_all_bit_rate; 
...
static inline void red_pipes_remove_drawable(Drawable *drawable)
{
    ...
    RING_FOREACH_SAFE(item, next, &drawable->pipes) {
        ...
        //spice_debug("222222222222222222222222222222222");
...
static void destroy_client_stream_global(RedWorker *worker)
{
    ...
    WORKER_FOREACH_DCC_SAFE(worker, item, next, dcc) {
        ...
        PipeItem * t_destroy_item= malloc(sizeof(PipeItem));
        red_channel_pipe_item_init(dcc, (PipeItem * )t_destroy_item, PIPE_ITEM_TYPE_STREAM_DESTROY);
        red_channel_client_pipe_add(&dcc->common.base, t_destroy_item);
...
#ifdef RED_STREAM_FULL_H264_CODE_THREADING 
                if (worker->threading->get_spice_encoding_threading_status(worker->threading) == RED_THREADING_RUNNING)
                {
                    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : beging to stop_wait_spice_encoding_threading",__FILE__,__FUNCTION__,__LINE__);
                    worker->threading->stop_wait_spice_encoding_threading(worker->threading);
                    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : end to stop_wait_spice_encoding_threading",__FILE__,__FUNCTION__,__LINE__);
                }
#endif 
...
{
        //spice_debug("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
                                                 ...
                                                 int wait_if_used)
{
    ...
    while ((item = (PipeItem *)ring_next(ring, (RingItem *)item))) {
        ...
        //spice_debug("333333333333333333333333333333333333333");
...
static void red_display_stream_agent_stop(DisplayChannelClient *dcc, StreamAgent *agent)
{
    ...
    //add by lcx for h264
    if (agent->h264_encoder)
    {
        h264_encoder_uninit(agent->h264_encoder);
        agent->h264_encoder = NULL;
    }
...
static void print_spice_bmp_fmt(SpiceBitmapFmt fmt)
{
     switch (fmt) {
        case SPICE_BITMAP_FMT_4BIT_BE:
            spice_debug("bitmap format %s", "SPICE_BITMAP_FMT_4BIT_BE");
            break;
        case SPICE_BITMAP_FMT_4BIT_LE:
            spice_debug("bitmap format %s", "SPICE_BITMAP_FMT_4BIT_LE");
            break;
        case SPICE_BITMAP_FMT_8BIT:
            spice_debug("bitmap format %s", "SPICE_BITMAP_FMT_8BIT");
            break;
        case SPICE_BITMAP_FMT_16BIT:
            spice_debug("bitmap format %s", "SPICE_BITMAP_FMT_16BIT");
            break;
        case SPICE_BITMAP_FMT_24BIT:
            spice_debug("bitmap format %s", "SPICE_BITMAP_FMT_24BIT");
            break;
        case SPICE_BITMAP_FMT_32BIT:
            spice_debug("bitmap format %s", "SPICE_BITMAP_FMT_32BIT");
            break;
        case SPICE_BITMAP_FMT_RGBA:
            spice_debug("bitmap format %s", "SPICE_BITMAP_FMT_RGBA");
            break;
        default:
            spice_error("invalid bitmap format  %u", fmt);
    }

}

static enum AVPixelFormat red_spicefmt_to_ffmpeg_fmt(SpiceBitmapFmt fmt) 
{
    enum AVPixelFormat ffmpeg_fmt;
    switch (fmt) {
        case SPICE_BITMAP_FMT_4BIT_BE:
        case SPICE_BITMAP_FMT_4BIT_LE:
            ffmpeg_fmt = AV_PIX_FMT_BGR4;
            break;
        case SPICE_BITMAP_FMT_8BIT:
            ffmpeg_fmt = AV_PIX_FMT_RGB8;
            break;
        case SPICE_BITMAP_FMT_16BIT:
            ffmpeg_fmt = AV_PIX_FMT_RGB565BE;
            break;
        case SPICE_BITMAP_FMT_24BIT:
            ffmpeg_fmt = AV_PIX_FMT_BGR24;
            break;
        case SPICE_BITMAP_FMT_32BIT:
        case SPICE_BITMAP_FMT_RGBA:
            ffmpeg_fmt = AV_PIX_FMT_BGRA;
            break;
        default:
            spice_error("invalid bitmap format  %u", fmt);
            ffmpeg_fmt = AV_PIX_FMT_NONE;
    }
    print_spice_bmp_fmt(fmt);
    return ffmpeg_fmt;
}


static void red_create_h264_stream(DisplayChannelClient *dcc, Stream *stream)
{
    StreamAgent *agent = &dcc->stream_agents[get_stream_id(dcc->common.worker, stream)];

    H264StreamInfo info;
    AVRational rate = { 1, stream->input_fps};
    ...
    Drawable *drawable  = stream->current;
    ...
    info.s_pix_fmt = red_spicefmt_to_ffmpeg_fmt(bitmap->format);
    GET_FORMAT_BITS(info.s_pix_fmt,n_pixel_bits); 
    //info.width =  ( (stream->width % 2) == 0 ) ? stream->width : (stream->width + 1);
    //info.height = ( (stream->height % 2) == 0 ) ? stream->height : (stream->height + 1);

    //info.width= (((stream->width * n_pixel_bits) + 31) / 32) * 4;
    //info.width= (stream->width + 3 ) / 4 * 4;
    info.width= (stream->width + 1 ) / 2 * 2;
    info.height = (stream->height + 1) / 2 * 2;

    //spice_debug(" === info.width = %d  info.height = %d   === n_pixel_bits = %d ==,stream->width = %d",info.width,info.height,n_pixel_bits, stream->width);
    ...
    agent->h264_encoder =  h264_encoder_new(); 
    h264_encoder_init(agent->h264_encoder,&info);
}
...
static void red_display_create_stream(DisplayChannelClient *dcc, Stream *stream)
{
    ...
    spice_info("red_display_create_stream ???");
                                ...
                                int last_gradual_frame)
{
    ...
    spice_info("red_stream_add_frame === ");
...
static inline int red_current_add_equal(RedWorker *worker, DrawItem *item, TreeItem *other)
{
    ...
    if (item->effect == QXL_EFFECT_OPAQUE) {
        ...
        //spice_debug("bbbbbbbbbbbbbbbbbbbbbbbbbbbb");
    ...
    case QXL_EFFECT_REVERT_ON_DUP:
        ...
        //spice_debug("cccccccccccccccccccccccccccccccccccccccc");
            red_pipes_remove_drawable(other_drawable);
...
static inline void red_use_stream_trace(RedWorker *worker, Drawable *drawable)
{
    ...
    for (; trace < trace_end; trace++) {
        if (__red_is_next_stream_frame(worker, drawable, trace->width, trace->height,
            ...
            spice_info("red_use_stream_trace");
    ...
    if (red_current_add_qxl(worker, &worker->surfaces[surface_id].current, drawable,
        ...
        if (worker->streaming_video_compression == SPICE_STREAM_VIDEO_COMPRESSION_H264_FULL)
        {
            ...
            if (worker->threading->get_spice_encoding_threading_status(worker->threading) == RED_THREADING_RUNNING)
            {
                ...
                worker->threading->read_bits_with_cb_to_encoding_threading( worker->threading,&area);
...
//add by lcx for init ffmpeg
static void red_init_more_video_compress(RedWorker *worker)
{
    h264_encoder_env_init();
    worker->more_video_compress = 1;
}
...
//#endif
    ...
    case SPICE_IMAGE_TYPE_QUIC:
    spice_info(" SPICE_IMAGE_TYPE_QUIC===================111111111111111111");
        red_display_add_image_to_pixmap_cache(rcc, simage, &image, FALSE);
...
{
    //spice_info(" red_marshall_qxl_draw_opaque ===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *brush_pat_out;
...
static void red_marshall_qxl_draw_transparent(RedWorker *worker,
                                          ...
                                          DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_transparent===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *src_bitmap_out;
...
static FillBitsType red_marshall_qxl_draw_alpha_blend(RedWorker *worker,
                                                  ...
                                                  int src_allowed_lossy)
{
    //spice_info(" red_marshall_qxl_draw_alpha_blend===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *src_bitmap_out;
...
static void red_marshall_qxl_copy_bits(RedWorker *worker,
                                   ...
                                   DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_copy_bits===================111111111111111111");
    Drawable *item = dpi->drawable;
    RedDrawable *drawable = item->red_drawable;
...
static void red_marshall_qxl_draw_blend(RedWorker *worker,
                                    ...
                                    DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_blend===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *src_bitmap_out;
...
static void red_marshall_qxl_draw_blackness(RedWorker *worker,
                                        ...
                                        DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_blackness===================111111111111111111");
    Drawable *item = dpi->drawable;
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *mask_bitmap_out;
...
static void red_marshall_qxl_draw_whiteness(RedWorker *worker,
                                        ...
                                        DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_whiteness===================111111111111111111");
    Drawable *item = dpi->drawable;
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *mask_bitmap_out;
...
static void red_marshall_qxl_draw_inverse(RedWorker *worker,
                                        ...
                                        Drawable *item)
{
    //spice_info(" red_marshall_qxl_draw_inverse===================111111111111111111");
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *mask_bitmap_out;
    SpiceInvers inverse;
...
static void red_marshall_qxl_draw_rop3(RedWorker *worker,
                                   ...
                                   DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_rop3===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceRop3 rop3;
...
static void red_marshall_qxl_draw_composite(RedWorker *worker,
                                     ...
                                     DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_composite===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceMarshaller *src_bitmap_out;
...
static void red_marshall_qxl_draw_stroke(RedWorker *worker,
                                     ...
                                     DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_stroke===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceStroke stroke;
...
static void red_marshall_qxl_draw_text(RedWorker *worker,
                                   ...
                                   DrawablePipeItem *dpi)
{
    //spice_info(" red_marshall_qxl_draw_text===================111111111111111111");
    Drawable *item = dpi->drawable;
    DisplayChannelClient *dcc = RCC_TO_DCC(rcc);
    RedDrawable *drawable = item->red_drawable;
    SpiceText text;
...
static int get_h264_encode_frame_buffer(DisplayChannelClient *dcc, const SpiceRect *src,
                        const SpiceBitmap *image, Stream *stream,char *outbuf,int *outsize)
{
    SpiceChunks *chunks;
    uint32_t image_stride;
    size_t offset;
    ...
    /*
    spice_debug("stream_height = %d   stream_width = %d codec_context->width = %d codec_context->height =%d",
            stream_height,stream_width,agent->h264_encoder->codec_context->width,
            agent->h264_encoder->codec_context->height);
*/
    
    
    int n_pixel_bits = h264_encoder_get_bits_per_pixel(agent->h264_encoder);
    int n_pixel_bytes = h264_encoder_get_bytes_per_pixel(agent->h264_encoder);
    int copy_bytes_size = SPICE_ALIGN(n_pixel_bits * stream_width, 8) / 8;
    //int row_size = agent->h264_encoder->codec_context->width * n_pixel_bytes;
    int row_size = (((agent->h264_encoder->codec_context->width * n_pixel_bits) + 31) / 32) * 4;

    /*
    spice_debug(" ===rgb_fmt = %d == row_size = %d  n_pixel_bits = %d copy_bytes_size = %d == n_pixel_bytes = %d",
            agent->h264_encoder->rgb_fmt,
            row_size,
            n_pixel_bits,copy_bytes_size,
            n_pixel_bytes);
*/


    char *pbuf = outbuf;
    memset(pbuf,0x00,*outsize);
    for (i = 0; i < stream_height; i++) {
        uint8_t *src_line = (uint8_t *)red_get_image_line(chunks, &offset, &chunk, image_stride);
        ...
        // only for 32 bit
        //src_line += src->left * mjpeg_encoder_get_bytes_per_pixel(agent->mjpeg_encoder);
        src_line += src->left * n_pixel_bytes; 

        
        memcpy(pbuf,src_line,copy_bytes_size);
        pbuf += copy_bytes_size;
        if(row_size > copy_bytes_size)
        {
//            spice_debug(" =============================  row_size > copy_bytes_size 1");
            pbuf += (row_size - copy_bytes_size);
        }
    }
    if(agent->h264_encoder->codec_context->height > stream_height)
    {
        pbuf += copy_bytes_size;
        if(row_size > copy_bytes_size)
        {
...
int bmp_generator1(int width,int height,unsigned char *data)
{
    ...
    sprintf(filename,"/home/lichenxiang/pic/test_%d.bmp",file_index);
    file_index++;

    n_pixel_bits = 32;

    
    row_size = (((width * n_pixel_bits) + 31) / 32) * 4;
    bitmap_data_offset = header_size;

    file_size = bitmap_data_offset + (height * row_size);
                  ...
                  SpiceMarshaller *base_marshaller, Drawable *drawable)
{
    ...
    if (drawable->sized_stream) {
        if (red_channel_client_test_remote_cap(rcc, SPICE_DISPLAY_CAP_SIZED_STREAM)) {
            ...
            spice_info("drawable->sized_stream ====================================================================");

            width = src_rect->right - src_rect->left;
            height = src_rect->bottom - src_rect->top;
            H264StreamInfo info;
            memset(&info,0,sizeof(H264StreamInfo));
            h264_encoder_get_stream_info(agent->h264_encoder,&info);
            //info.width = ((width % 2) == 0) ? width : (width + 1);
            //info.height = ((height % 2) == 0) ? height : (height + 1);
            
            int n_pixel_bits;
            info.s_pix_fmt = red_spicefmt_to_ffmpeg_fmt(image->u.bitmap.format);
            GET_FORMAT_BITS(info.s_pix_fmt,n_pixel_bits); 
            ...
            int ret = h264_encoder_reinit(agent->h264_encoder,&info);
            if(ret == H264_ERROR)
            {
                spice_info("h264_encoder_reinit error ================");
                return FALSE;            
            }

            spice_info("drawable->sized_stream === width = %d ,height = %d",width,height);
        } else {
            return FALSE;
        }
    } else {
    ...
    spice_info("agent->h264_encoder =0x%x , agent = 0x%x .stream = 0x%x ,h264 width = %d , height = %d\n",
            agent->h264_encoder,agent,stream,
            width , height);
...
/*
    ...
    switch (image->u.bitmap.format) {
        case SPICE_BITMAP_FMT_32BIT:
        case SPICE_BITMAP_FMT_RGBA:
            spice_debug("SPICE_BITMAP_FMT_32BIT  SPICE_BITMAP_FMT_RGBA");
            break;
        case SPICE_BITMAP_FMT_16BIT:
            spice_debug("SPICE_BITMAP_FMT_16BIT");
            break;
        case SPICE_BITMAP_FMT_24BIT:
            spice_debug("SPICE_BITMAP_FMT_24BIT");
            break;
        default:
            spice_debug("unsupported format %d", image->u.bitmap.format);
...
*/
    ...
    //spice_debug("222222222222222222222222222222");
    char *bitmap_rgb = malloc(MAX_RGB_SIZE);
    int bitmap_rgb_size = MAX_RGB_SIZE;

    get_h264_encode_frame_buffer(dcc, &drawable->red_drawable->u.copy.src_area,&image->u.bitmap, 
            stream,bitmap_rgb,&bitmap_rgb_size);
   

    //sleep(1);
    //usleep(10*2000);
    ret = h264_encode(agent->h264_encoder,bitmap_rgb,bitmap_rgb_size,dcc->send_data.stream_outbuf,&outbuf_size);
    
    //spice_debug("?????????????333333333333333333333333333333");
//    bmp_generator1(agent->h264_encoder->codec_context->width,agent->h264_encoder->codec_context->height,bitmap_rgb);
    // dump_bitmap(&image->u.bitmap);
    //spice_debug("333333333333333333333333333333");
    free(bitmap_rgb);
    ...
    {
        //spice_debug("===========  H264_ENCODER_OK");

#if 0
        dcc->send_data.stream_outbuf_size = outbuf_size;
        char filename[1000] = "";
        sprintf(filename,"/home/lichenxiang/stream_before/h264_0x%xh264",agent->h264_encoder);
    ...
    //spice_debug("44444444444444444444444444");
    if (!drawable->sized_stream) {
    ...
    //spice_debug("5555555555555555555555555555555555");
        ...
        SpiceMarshaller *base_marshaller, Drawable *drawable)
{
    ...
    StreamAgent *agent = &dcc->stream_agents[get_stream_id(worker, stream)];
    spice_info("agent->mjpeg_encoder =0x%x , agent = 0x%x .stream = 0x%x ,mjpeg width = %d , height = %d\n",
            agent->mjpeg_encoder,agent,stream,
            width , height);
    uint64_t time_now = red_now();
    size_t outbuf_size;
...
static inline void marshall_qxl_h264_global(RedChannelClient *rcc,
        SpiceMarshaller *m)
{
    //add by lcx for h264
    uint32_t frame_mm_time;
    ...
    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : %ld.%ld  ",__FILE__,__FUNCTION__,__LINE__,time_now / (1000 * 1000 * 1000), time_now % (1000 * 1000 * 1000));
    
    H264DataItem * h264_data_item = worker->threading->remove_h264_slice_from_encoding_threading(worker->threading);
    if (h264_data_item)
    {
        spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : ?? %ld.%ld ?? \n\n ",__FILE__,__FUNCTION__,__LINE__,time_now / (1000 * 1000 * 1000), time_now % (1000 * 1000 * 1000));
        n = h264_data_item->size;
        memcpy(dcc->send_data.stream_outbuf,h264_data_item->slice,h264_data_item->size);
        SpiceMsgDisplayStreamData stream_data;
        red_channel_client_init_send_data(rcc, SPICE_MSG_DISPLAY_STREAM_DATA, NULL);
        ...
        free_h264_data_item(h264_data_item);
    }
    return ;
}
        ...
        SpiceMarshaller *m)
{
    ...
    if (time_now - last_send_time < (1000 * 1000 * 1000) / worker->stream_video_h264_all_fps) {
        ...
        if (worker->flush_frame_timer)
        {
            ...
            //spice_timer_set(worker->flush_frame_timer,(1000 /  worker->stream_video_h264_all_fps) - (time_now - last_send_time));
...
#endif
    spice_printerr("=================  %ld.%ld ========================",time_now / (1000*1000*1000),time_now % (1000*1000*1000));
    ...
    if (H264_NOT_GET_SLICE == ret)
    {
        if (worker->flush_frame_timer)
        {
            ...
            //spice_timer_set(worker->flush_frame_timer,RED_GLOBAL_STREAM_TIMOUT);
            //spice_timer_set(worker->flush_frame_timer,1000 /  worker->stream_video_h264_all_fps);
            spice_timer_set(worker->flush_frame_timer,  1);
#endif
        }
...
{
    //spice_info("display_channel_send_item = 0x%x",display_channel_send_item);
    ...
    break;
    case PIPE_ITEM_TYPE_STREAM_THREADING:{
            //spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : come send PIPE_ITEM_TYPE_STREAM_THREADING",__FILE__,__FUNCTION__,__LINE__);
            marshall_qxl_h264_global(rcc, m);
    }
    break;
#endif
    ...
    case PIPE_ITEM_TYPE_DRAW: {
        ...
        //{
            ...
            {
#ifndef RED_STREAM_FULL_H264_CODE_TIMER
#ifndef RED_STREAM_FULL_H264_CODE_THREADING
                spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : %s",__FILE__,__FUNCTION__,__LINE__,
                        "wocaocoacaocaocaocaocaocaocaocao ");
    ...
    if (pipe_item->type == PIPE_ITEM_TYPE_DRAW)
    {
        ...
        if(type == SPICE_MSG_DISPLAY_STREAM_DATA_SIZED || type == SPICE_MSG_DISPLAY_STREAM_DATA)
        {
            ...
            RedWorker *worker = dcc->common.worker;
            Drawable *item = dpi->drawable;
            Stream *stream = dpi->drawable->stream;
            StreamAgent *agent = &dcc->stream_agents[get_stream_id(worker, stream)];
            char filename[1000] = "";
            sprintf(filename,"/home/lichenxiang/stream/h264_0x%xh264",agent->h264_encoder);
            FILE *fp = fopen(filename,"a");
            fwrite(dcc->send_data.stream_outbuf,1,dcc->send_data.stream_outbuf_size,fp);
            fclose(fp);
        }
...
/*
            ...
            if(type == SPICE_MSG_DISPLAY_STREAM_DATA_SIZED || type == SPICE_MSG_DISPLAY_STREAM_DATA)
            {
                ...
                StreamAgent *agent = &dcc->stream_agents[get_stream_id(worker, stream)];
                char filename[1000] = "";
                sprintf(filename,"/home/lichenxiang/stream_after/h264_0x%xh264",agent->h264_encoder);
                FILE *fp = fopen(filename,"a");
                fwrite(dcc->send_data.stream_outbuf,1,dcc->send_data.stream_outbuf_size,fp);
                fclose(fp);
            }
...
static void display_channel_client_on_disconnect(RedChannelClient *rcc)
{
    ...
    if (worker->streaming_video_compression == SPICE_STREAM_VIDEO_COMPRESSION_H264_FULL)
    {
        spice_printerr("begin to free worker->streaming_video_compression!!!");
        if (worker->g_h264_encoder != NULL)
        {
            h264_encoder_uninit(worker->g_h264_encoder);
        }
...
static void create_client_stream_global(RedWorker *worker)
{
    ...
    RingItem *item, *next;
    WORKER_FOREACH_DCC_SAFE(worker, item, next, dcc) {
        PipeItem * t_create_item= malloc(sizeof(PipeItem));
        red_channel_pipe_item_init(dcc, (PipeItem * )t_create_item, PIPE_ITEM_TYPE_STREAM_CREATE);
        red_channel_client_pipe_add(&dcc->common.base, t_create_item);
...
static void red_client_stream_timout_global(RedWorker *worker)
{
    ...
    WORKER_FOREACH_DCC_SAFE(worker, item, next, dcc) {
       /* 
        ...
        {
            spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] prev = 0x%x  , netx = 0x%x:  ==  == ??",
                    __FILE__,__FUNCTION__,__LINE__,
            ...
            RedChannelClient *rcc = &dcc->common.base;
            rcc->pipe_size--;
            ring_remove(&worker->g_stream_timout_item.link);

        }
*/
        TimOutItem *stream_timout_item = malloc(sizeof(TimOutItem));
        red_channel_pipe_item_init(dcc, (PipeItem * )stream_timout_item , PIPE_ITEM_TYPE_STREAM_TIMOUT);
        //red_channel_client_pipe_add(&dcc->common.base, &worker->g_stream_timout_item);
        red_channel_client_pipe_add(&dcc->common.base, stream_timout_item);
    }
}
...
static void* encoding_cb(void* ctx,void * ud)    
{
    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : begin",__FILE__,__FUNCTION__,__LINE__);
    
    encoding_dispatcher_data_ready_event(1);
}

static void* encoding_read_bits_cb(void* ctx,void * ud)    
{
    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : begin",__FILE__,__FUNCTION__,__LINE__);
    ...
    if (worker->threading->get_spice_encoding_threading_status(worker->threading) == RED_THREADING_RUNNING)
    {
        VedioDataItem *vedio_data = (VedioDataItem *)ud;
        red_get_primary_surface_bits(worker,  vedio_data->bits);
    }
}

static void* encoding_get_surface_cb(void* ctx,void * ud)    
{
    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : begin",__FILE__,__FUNCTION__,__LINE__);
    
    encoding_dispatcher_get_surface_event(1);
}
...
static inline void red_create_surface_item(DisplayChannelClient *dcc, int surface_id)
{
    ...
    if (worker->streaming_video_compression == SPICE_STREAM_VIDEO_COMPRESSION_H264_FULL)
    {
        if (flags & SPICE_SURFACE_FLAGS_PRIMARY)
        {
            ...
            if (worker->threading->get_spice_encoding_threading_status(worker->threading) == RED_THREADING_RUNNING)
            {
                spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : 111111111111 beging to stop_wait_spice_encoding_threading",__FILE__,__FUNCTION__,__LINE__);
                worker->threading->stop_wait_spice_encoding_threading(worker->threading);
                spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : 22222222222222 end to stop_wait_spice_encoding_threading",__FILE__,__FUNCTION__,__LINE__);
            }
            int bpp = 0;
            GET_FORMAT_BYTES(AV_PIX_FMT_BGRA,bpp) ;
            LCXCALLBACK call_sending;
            call_sending._cb = encoding_cb;
            call_sending._ctx = worker;
           
            LCXCALLBACK encoding_read_bits;
            encoding_read_bits._cb = encoding_read_bits_cb;
            encoding_read_bits._ctx = worker;

            LCXCALLBACK encoding_get_surface;
            encoding_get_surface._cb = encoding_get_surface_cb;
            encoding_get_surface._ctx = worker;
            
            SpiceRect bbox = {0,0,worker->surfaces[0].context.width,worker->surfaces[0].context.height};
            worker->threading->init_spice_encoding_threading(worker->threading,
                    worker->surfaces[0].context.width * worker->surfaces[0].context.height * bpp ,
                    &bbox,
                    &call_sending,
                    &encoding_read_bits,
                    &encoding_get_surface,
                    AV_PIX_FMT_BGRA,
                    dcc->stream_video_h264_all_fps,
                    dcc->stream_video_h264_all_bit_rate);
            worker->threading->start_spice_encoding_threading(worker->threading);
#endif 
...
static SpiceWatch *encoding_worker_watch_add(int fd, int event_mask, SpiceWatchFunc func, void *opaque)
{
    /* Since we are a channel core implementation, we always get called from
       red_channel_client_create(), so opaque always is our rcc */
    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : begin",__FILE__,__FUNCTION__,__LINE__);
    struct RedWorker *worker = (struct RedWorker * )opaque;
    int i;
    ...
    if (i == MAX_EVENT_SOURCES) {
        spice_warning("could not add a watch for channel type ============== lcx");
        return NULL;
    }
...
SpiceCoreInterface encoding_worker_core = {
    .timer_add = spice_timer_queue_add,
    .timer_start = spice_timer_set,
    .timer_cancel = spice_timer_cancel,
    .timer_remove = spice_timer_remove,
    ...
    .watch_update_mask = worker_watch_update_mask,
    .watch_add = encoding_worker_watch_add,
    .watch_remove = worker_watch_remove,
};
...
//add by lcx for h264
void handle_dev_set_streaming_video_compression(void *opaque, void *payload)
{
    RedWorkerMessageSetStreamingVideoCompression *msg = payload;
    RedWorker *worker = opaque;

    spice_printerr("\n\ni come to handle_dev_set_streaming_video_compression = %d\n\n",msg->streaming_video_compression);
    worker->streaming_video_compression = msg->streaming_video_compression;
    spice_assert(worker->streaming_video_compression != STREAM_VIDEO_COMPRESSION_INVALID);

    switch(worker->streaming_video_compression) {
        case STREAM_VIDEO_COMPRESSION_MJPEG:
            spice_info("svc mjpeg");
            break;
        case STREAM_VIDEO_COMPRESSION_H264:
            spice_info("svc h264");
            break;
        default:
...
static void register_callbacks(Dispatcher *dispatcher)
{
    ...
    dispatcher_register_handler(dispatcher,
                                ...
                                handle_dev_set_streaming_video_compression,
...
static void red_client_stream_h264_threading_global(RedWorker *worker)
{
    //spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : ",__FILE__,__FUNCTION__,__LINE__);
    DisplayChannelClient *dcc;
    RingItem *item, *next;
    ...
    //remove_h264_slice_from_encoding_threading
    WORKER_FOREACH_DCC_SAFE(worker, item, next, dcc) {
        H264ThreadingItem *h264_item = malloc(sizeof(H264ThreadingItem)); 
        red_channel_pipe_item_init(dcc, (PipeItem * )h264_item, PIPE_ITEM_TYPE_STREAM_THREADING);
        red_channel_client_pipe_add(&dcc->common.base, h264_item);
    }
}


void* encoding_dispatcher_cb(void* ctx,void * ud)
{
    ...
    if(worker->threading->get_spice_encoding_threading_status(worker->threading) == RED_THREADING_RUNNING)
    {
        red_client_stream_h264_threading_global(worker);
        red_channel_push(&worker->display_channel->common.base);
    }
}


void* encoding_dispatcher_get_surface_cb(void* ctx,void * ud)
{
//    spice_printerr("[FILE : %s][FUNCTION : %s][LINE : %d] : begin",__FILE__,__FUNCTION__,__LINE__);
    ...
    if (worker->threading->get_spice_encoding_threading_status(worker->threading) == RED_THREADING_RUNNING)
    {
        int bpp = 0;
        GET_FORMAT_BYTES(AV_PIX_FMT_BGRA,bpp) ;
        char * bits = spice_new(char ,worker->surfaces[0].context.width * worker->surfaces[0].context.height * bpp);
        red_get_primary_surface_bits(worker,bits);
        worker->threading->insert_vedio_data_in_encoding_threading(worker->threading,bits);
    }
}
...
SPICE_GNUC_NORETURN void *red_worker_main(void *arg)
{
    ...
    {
        worker->g_h264_encoder = NULL;
        worker->threading = NULL;
        worker->threading = encoding_threading_new();
        if (!worker->threading)
        {
            spice_error("encoding_threading_new");
        }
    }

    LCXCALLBACK dis_cb[ENCODING_DISPATCHER_CB_NUM];
    memset(dis_cb,0,sizeof(LCXCALLBACK) * ENCODING_DISPATCHER_CB_NUM);
    dis_cb[ENCODING_DISPATCHER_DATA_READY_CB]._cb = encoding_dispatcher_cb;
    dis_cb[ENCODING_DISPATCHER_DATA_READY_CB]._ctx = (void *)worker;

    dis_cb[ENCODING_DISPATCHER_GET_SURFACE_CB]._cb = encoding_dispatcher_get_surface_cb;
    dis_cb[ENCODING_DISPATCHER_GET_SURFACE_CB]._ctx = (void *)worker->threading;
    encoding_dispatcher_init(&encoding_worker_core,dis_cb,worker);
...
#ifdef RED_STREAM_FULL_H264_CODE_TIMER
        clock_gettime(CLOCK_MONOTONIC, &time);
        streams_time_out_all = 1000 / worker->stream_video_h264_all_fps - (timespec_to_red_time(&time) - now);
        if(streams_time_out_all <= 0)
        {
            streams_time_out_all = 0;
        }
#endif 
