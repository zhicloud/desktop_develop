...
{
    int ret;
    //add by lcx for igong sigpipe
    struct sigaction sa;
    sa.sa_handler = SIG_IGN;
    sa.sa_flags = 0;
    if (sigaction(SIGPIPE, &sa, 0) == -1) {
        spice_error("catch SIGPIPE error = %s ", strerror(errno));
    }
...
//add by lcx 
SPICE_GNUC_VISIBLE int spice_server_set_streaming_compression(SpiceServer *s, int value)
{
