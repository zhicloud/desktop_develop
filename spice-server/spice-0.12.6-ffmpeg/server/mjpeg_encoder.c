...
static inline void mjpeg_encoder_eval_quality(MJpegEncoder *encoder)
{
    ...
    spice_return_if_fail(rate_control->during_quality_eval);
...
static void mjpeg_encoder_adjust_params_to_bit_rate(MJpegEncoder *encoder)
{
    ...
    spice_return_if_fail(rate_control_is_active(encoder));
...
//    spice_info("start mjpeg frame, %d x %d, row size %d,  %d byte(s) per pixel",
//    						encoder->cinfo.image_width, encoder->cinfo.image_height, 
//    						encoder->row_size, encoder->bytes_per_pixel);
...
//    spice_info("encode frame, source region (%d, %d) ~ (%d, %d), top down:%d",
//    						src->left, src->top, src->right, src->bottom, top_down);
//    spice_info("image format %d, %d x %d, stride %d, %d byte(s) data, %d chunks",
//    						image->format, image->x, image->y, image_stride, chunks->data_size, chunks->num_chunks);
...
//		spice_info("%d line(s) written", stream_height);
    return TRUE;
}
