...
void red_dispatcher_on_vc_change(void)
{
    ...
    while (now) {
        now->qxl->st->qif->set_compression_level(now->qxl, compression_level);
        spice_info("codecs changed, %d codec", num_video_codecs);
...
void red_dispatcher_init(QXLInstance *qxl)
{
    ...
    red_channel_set_cap(channel, SPICE_DISPLAY_CAP_MULTI_CODEC);
    red_channel_set_cap(channel, SPICE_DISPLAY_CAP_CODEC_H264);
    red_channel_set_cap(channel, SPICE_DISPLAY_CAP_CODEC_MJPEG);
