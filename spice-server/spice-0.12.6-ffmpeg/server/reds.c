...
static EnumNames renderer_names[] = {
    {RED_RENDERER_SW, "sw"},
#ifdef USE_OPENGL
...
static int red_set_video_codecs(const char *codecs)
{
    uint32_t encoder;
    ...
    while ( (c = parse_video_codecs(c, &encoder_name, &codec_name)) ) {
        ...
        if (!encoder_name || !codec_name) {
            ...
            skip = TRUE;
				...
				spice_info("codec %s:%s parsed", encoder_name, codec_name);
        free(encoder_name);
        free(codec_name);
        if (skip) {
            continue;
...
SPICE_GNUC_VISIBLE int spice_server_init(SpiceServer *s, SpiceCoreInterface *core)
{
    ...
    //force enable codec
    //red_set_video_codecs("spice:mjpeg;gstreamer:h264");
    red_set_video_codecs("ffmpeg:h264");
    return ret;
}
...
SPICE_GNUC_VISIBLE int spice_server_set_video_codecs(SpiceServer *s, const char* video_codecs)
{
    ...
    if (!red_set_video_codecs(video_codecs)) {
        return -1;
    }
