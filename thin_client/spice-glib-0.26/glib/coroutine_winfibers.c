...
{
    //modify by lcx for yielding to no one 
    int isNull = 0;
    struct coroutine *to = coroutine_self()->caller;
    if (!to) {
        fprintf(stderr, "Co-routine is yielding to no one , so i try to call the main loop\n");
        abort();
        isNull = 1;
    }
    if(isNull)
    {
        to = &leader;
        if(coroutine_is_main(coroutine_self()))
        {
            fprintf(stderr, "Co-routine return NULL\n");
            return NULL;
        }
    }
