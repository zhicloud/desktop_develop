...
SpiceChannel *spice_channel_new(SpiceSession *s, int type, int id)
{
	...
	g_debug("SPICE_CHANNEL_MAIN: ==============================");
        break;
    case SPICE_CHANNEL_DISPLAY:
	g_debug("SPICE_CHANNEL_DISPLAY:: ==============================");
        gtype = SPICE_TYPE_DISPLAY_CHANNEL;
        break;
    case SPICE_CHANNEL_CURSOR:
	g_debug("SPICE_CHANNEL_CURSOR::: ==============================");
        gtype = SPICE_TYPE_CURSOR_CHANNEL;
        break;
    case SPICE_CHANNEL_INPUTS:
	g_debug("SPICE_CHANNEL_INPUTS::: ==============================");
	...
	g_debug("SPICE_CHANNEL_USBREDIR: 111111111111111111111111111111111111111111");
        if (!s->priv->usbredir) {
            g_debug("usbredir channel is disabled, not creating it");
            return NULL;
        }
	g_debug("SPICE_CHANNEL_USBREDIR: 222222222222222222222222222222222222222");
...
//add by lcx
static void setKeepAlive(SpiceChannel *channel, int iSockfd , socklen_t iIdleTime , socklen_t iInterval , socklen_t iCount )
{
    int keepAlive = 1;//设定KeepAlive
    int rc ;
    CHANNEL_DEBUG(channel, "setKeepAlive \n\n" );
    rc = setsockopt( iSockfd , SOL_SOCKET , SO_KEEPALIVE , (const char*)&keepAlive, sizeof(keepAlive) );
    if(rc < 0)
    {
        CHANNEL_DEBUG(channel, "SO_KEEPALIVE error" );
    }
    rc = setsockopt( iSockfd , SOL_TCP , TCP_KEEPIDLE , (const char*)&iIdleTime , sizeof(iIdleTime) );

    if(rc < 0)
    {
        CHANNEL_DEBUG(channel, "TCP_KEEPIDLE error" );
    }

    rc = setsockopt( iSockfd , SOL_TCP , TCP_KEEPINTVL , (const char*)&iInterval,sizeof(iInterval) );

    if(rc < 0)
    {
        CHANNEL_DEBUG(channel, "TCP_KEEPINTVL error" );
    }

    rc = setsockopt( iSockfd , SOL_TCP , TCP_KEEPCNT , (const char*)&iCount,sizeof(iCount) );
    
    if(rc < 0)
    {
        CHANNEL_DEBUG(channel, "TCP_KEEPCNT  error" );
    }
}
