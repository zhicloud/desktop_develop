...
void spice_session_dispose_self(SpiceSession *session)
{
    SpiceSession *session1 = SPICE_SESSION(session);

    SpiceSessionPrivate *s = session1->priv;
    g_clear_object(&s->audio_manager);
}

void spice_session_dispose_self_cache(SpiceSession *session)
{
    SpiceSession *session1 = SPICE_SESSION(session);

    cache_clear_all(session1);
}
...
//    printf("[file :%s][fun : %s][line : %d] wocaowocaowocaowocao\n",__FILE__,__FUNCTION__,__LINE__);
    for (ring = ring_get_head(&s->channels); ring != NULL; ring = next) {
        next = ring_next(&s->channels, ring);
        item = SPICE_CONTAINEROF(ring, struct channel, link);
