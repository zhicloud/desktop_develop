...
G_DEFINE_TYPE(SpiceDisplay, spice_display, G_TYPE_OBJECT);
...
static void primary_create(SpiceChannel *channel, gint format, gint width, gint height, gint stride, gint shmid, gpointer imgdata, gpointer data) {
    ...
    //SPICE_LOG("111111111111111%d ================== %d ==================== %d ==================== %d\n",0,width,height,4);
    uiCallbackSettingsChanged (display,0, width, height, 4);
}
...
bool display2conn(spice_connection** tmp_conn,SpiceDisplay *display)
{
    int i = 0;
    for(; i < MAX_SPICE_CONNECT; i++)
    {
        if(!global_wsgi->SpiceConnList[i])
	{
	 continue;
	}
	*tmp_conn = global_wsgi->SpiceConnList[i]->conn;
        if(*tmp_conn)
        {
            if ((*tmp_conn)->global_display == display)
            {
                break;
            }
        ...
        gpointer rgba, gpointer data)
{
    ...
    { 
        ...
        {
            SPICE_CursorSet mycur = {width,height,hot_x,hot_y,rgba};
            tmp_conn->callback[CB_CURSOr_SET].lcb._ud = &mycur;
            if(tmp_conn->callback[CB_CURSOr_SET].lcb._cb)
            {
                tmp_conn->callback[CB_CURSOr_SET].lcb._cb(
                        tmp_conn->callback[CB_CURSOr_SET].lcb._ctx,
                        tmp_conn->callback[CB_CURSOr_SET].lcb._ud);
            }
        }
    }
...
static void cursor_move(SpiceCursorChannel *channel, gint x, gint y, gpointer data) 
{
    ...
    { 
        if(tmp_conn->callback)
        {
            SPICE_CursorMove pos = {x,y};
            tmp_conn->callback[CB_CURSOR_MOVE].lcb._ud = &pos;
            if(tmp_conn->callback[CB_CURSOR_MOVE].lcb._cb)
            {
                tmp_conn->callback[CB_CURSOR_MOVE].lcb._cb(
                        tmp_conn->callback[CB_CURSOR_MOVE].lcb._ctx,
                        tmp_conn->callback[CB_CURSOR_MOVE].lcb._ud);
            }
        }
    }
    g_rec_mutex_unlock(&global_wsgi->rec_mutex_conn);
...
static void cursor_hide(SpiceCursorChannel *channel, gpointer data) 
{
    ...
    { 
        if(tmp_conn->callback)
        {
            tmp_conn->callback[CB_CURSOR_HIDE].lcb._ud = NULL;
            if(tmp_conn->callback[CB_CURSOR_HIDE].lcb._cb)
            {
                tmp_conn->callback[CB_CURSOR_HIDE].lcb._cb(
                        tmp_conn->callback[CB_CURSOR_HIDE].lcb._ctx,
                        tmp_conn->callback[CB_CURSOR_HIDE].lcb._ud);
            }
        }
    }
    g_rec_mutex_unlock(&global_wsgi->rec_mutex_conn);
...
static void cursor_reset(SpiceCursorChannel *channel, gpointer data)
{
    ...
    { 
        if(tmp_conn->callback)
        {
            tmp_conn->callback[CB_CURSOR_RESET].lcb._ud = NULL;
            if(tmp_conn->callback[CB_CURSOR_RESET].lcb._cb)
            {
                tmp_conn->callback[CB_CURSOR_RESET].lcb._cb(
                        tmp_conn->callback[CB_CURSOR_RESET].lcb._ctx,
                        tmp_conn->callback[CB_CURSOR_RESET].lcb._ud);
            }
        }
    }
    g_rec_mutex_unlock(&global_wsgi->rec_mutex_conn);
