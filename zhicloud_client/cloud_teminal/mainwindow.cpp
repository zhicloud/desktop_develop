...
: QMainWindow(parent)
{
	...
	netdicwidget->setFixedSize(340, 60);
	netdicwidget->setGeometry((currentScreenWidth - 340) / 2, (currentScreenHeight - 60) / 2, 340, 60);
	netdicwidget->hide();
...
void CMainWindow::dhcpclickfunc()
{
	...
	ipaddredit->GetEdit()->setReadOnly(true);
	maskedit->GetEdit()->setReadOnly(true);
	gatewayedit->GetEdit()->setReadOnly(true);
	...
	if (0 == isdhcpclick)
	{
		...
		ip = mask = gateway = QStringLiteral("正在获取中...");
		ipaddredit->GetEdit()->setText(ip);
		maskedit->GetEdit()->setText(mask);
		gatewayedit->GetEdit()->setText(gateway);
...
void CMainWindow::createNetSettingWidget()
{
	...
	netwidget->setFixedSize(350, 350);
	QVBoxLayout* netmainl = new QVBoxLayout;
	netmainl->setContentsMargins(20, 30, 0, 10);
	...
	centertop->setFixedSize(250, 130);
	QLabel* ipaddr = new QLabel;
	ipaddr->setFixedSize(42, 14);
	ipaddr->setStyleSheet("background-color:transparent;color:rgb(228,228,228);font-size:12px;border:0px;");
	ipaddr->setText(QStringLiteral("IP地址:"));
	...
	mask->setFixedSize(52, 14);
	mask->setStyleSheet("background-color:transparent;color:rgb(228,228,228);font-size:12px;border:0px;");
	mask->setText(QStringLiteral("子网掩码:"));
	maskedit = new CMyTextEdit(":/setsvrurl/input", "", 150, 30, 1);
	maskedit->GetEdit()->setText(netmask);
	QLabel* gateway = new QLabel;
	gateway->setFixedSize(30, 14);
	gateway->setStyleSheet("background-color:transparent;color:rgb(228,228,228);font-size:12px;border:0px;");
	gateway->setText(QStringLiteral("网关:"));
	...
	c_layout->addWidget(ipaddr, 0, 0, Qt::AlignRight);
	c_layout->addWidget(ipaddredit, 0, 1);
	c_layout->addWidget(mask, 1, 0, Qt::AlignRight);
	c_layout->addWidget(maskedit, 1, 1);
	c_layout->addWidget(gateway, 2, 0, Qt::AlignRight);
	c_layout->addWidget(gatewayedit, 2, 1);
	centertop->setLayout(c_layout);


	QWidget* centerbottom = new QWidget;
	centerbottom->setFixedSize(250, 100);
	QLabel* svrurl = new QLabel;
	...
	centerbottom->setLayout(cblayout);
	...
	netmainl->addLayout(h_toplayout);
	netmainl->addWidget(centertop);
	netmainl->addSpacing(50);
...
void CMainWindow::createAboutWidget()
{
	...
	hardwareverson->setText(QStringLiteral("固件版本: FW1.0.2"));
...
void CMainWindow::replyFinished(QNetworkReply* reply)
{
	...
	else
	{
		...
		if (m_cmd == Login)
		{
			...
			msgbox = new ZCMessageBox(QStringLiteral("用户名或者密码错误,请重试"), this, NULL);
			msgbox->show();
		...
		MyZCLog::Instance().WriteToLog(ZCERROR, QString("QNetworkReply::Error cmd=") + QString("%1").arg(m_cmd) + QString(" error"));
	}
	reply->close();
}
...
bool CMainWindow::parseResult(ZcCommandType type, QByteArray data)
{
	...
	case Auth:
	{
				 ...
				 MyZCLog::Instance().WriteToLog(ZCINFO, QString() + QString("Auth=1 ") + QString(":") + data);
				 if (ret)
				 {
					 QSettings* settings = new QSettings(INIFILE, QSettings::IniFormat);
					 if (settings)
	...
	case Login:
	{
				  ...
				  MyZCLog::Instance().WriteToLog(ZCINFO, QString() + QString("Login=2 ") + QString(":") + data);
				  if (ret)
				  {
					  ...
					  else if (m_hostinfo.size() == 1)
					  {
						  ...
						  if (it != m_hostinfo.end())
						  {
							  ...
							  if (QString("%1").arg(1) == (it->second)["running_status"])
							  {
								  ismultihost = false;
								  isstarthost = true;
	...
	case ChangePWD:
		{
					  ret = parseLoginRet(data);
					  MyZCLog::Instance().WriteToLog(ZCINFO, QString() + QString("ChangePWD=1 ") + QString(":") + data);
					  if (ret)
					  {
						  if (NULL != msgbox)
						  {
	...
	case StartHost:
		{
						...
						MyZCLog::Instance().WriteToLog(ZCINFO, QString() + QString("StartHost=1 ") + QString(":") + data);
						printf(data);
						ret = 1;
                                                if (ret)
                                                {
	...
	case StopHost:
		{
					 ret = parseOperateRet(data);
					 MyZCLog::Instance().WriteToLog(ZCINFO, QString() + QString("StopHost=6 ") + QString(":") + data);
					 if (!ret)
					 {
						 if (NULL != msgbox)
						 {
	...
	case RestartHost:
		{
						ret = parseOperateRet(data);
						MyZCLog::Instance().WriteToLog(ZCINFO, QString() + QString("RestartHost=1 ") + QString(":") + data);
						if (ret)
						{
							if (NULL != msgbox)
							{
...
void CMainWindow::turnleft()
{
	...
	cardarray[0]->SetUname(*(*(USERCARDINFO*)prior->data).username);
	cardarray[1]->SetUname(*(*(USERCARDINFO*)cur->data).username);
	cardarray[2]->SetUname(*(*(USERCARDINFO*)next->data).username);
}
...
void CMainWindow::deletecard(CUserCard* card)
{
	...
	if (card == cardarray[0])
	{
		...
		card->SetUname(*(*(USERCARDINFO*)prior->data).username);
	}
	if (card == cardarray[2])
	{
		next = list_nextelem(userlist, cur->data, &elem);
		...
		card->SetUname(*(*(USERCARDINFO*)next->data).username);
	}
...
void CMainWindow::setChangeUserPic(QWidget* widget)
{
	if (widget == morethan3Widget)
	{
		...
		cardarray[0]->SetUname(*(*(USERCARDINFO*)cur->prior->data).username);
		cardarray[1]->setPixmap(*(*(USERCARDINFO*)cur->data).logopath);
		cardarray[1]->SetUname(*(*(USERCARDINFO*)cur->data).username);
		cardarray[2]->setPixmap(*(*(USERCARDINFO*)cur->next->data).logopath);
		cardarray[2]->SetUname(*(*(USERCARDINFO*)cur->next->data).username);
	...
	if (widget == User1Widget)
	{
		...
		cardarray[8]->SetUname(*(*(USERCARDINFO*)cur->data).username);
	}
	if (widget == User2Widget)
	{
		cur = userlist->next->next;
		cardarray[6]->setPixmap(*(*(USERCARDINFO*)cur->prior->data).logopath);
		cardarray[6]->SetUname(*(*(USERCARDINFO*)cur->prior->data).username);
		cardarray[7]->setPixmap(*(*(USERCARDINFO*)cur->data).logopath);
		cardarray[7]->SetUname(*(*(USERCARDINFO*)cur->data).username);
	...
	if (widget == User3Widget)
	{
		...
		cardarray[3]->SetUname(*(*(USERCARDINFO*)cur->prior->data).username);
		cardarray[4]->setPixmap(*(*(USERCARDINFO*)cur->data).logopath);
		cardarray[4]->SetUname(*(*(USERCARDINFO*)cur->data).username);
		cardarray[5]->setPixmap(*(*(USERCARDINFO*)cur->next->data).logopath);
		cardarray[5]->SetUname(*(*(USERCARDINFO*)cur->next->data).username);
...
int CMainWindow::spicelogin(QString ip, QString port, QString acc, QString pwd)
{
	...
	if (ret)
	{
		...
		MyZCLog::Instance().WriteToLog(ZCINFO, QString("openspice return value is ") + QString("%1").arg(iR));
...
//netdianose end
	case ORI_DIS_EVENT:
	{
		...
		MyZCLog::Instance().WriteToLog(ZCINFO, QString("viewer show now "));
		if (!viewer)
		{
			break;
		}
	...
	{
		MyZCLog::Instance().WriteToLog(ZCINFO, QString("spice channel error link "));
		if (NULL != msgbox)
		{
			delete msgbox;
			msgbox = NULL;
		 }
		msgbox = new ZCMessageBox(QStringLiteral("主机已关闭"), this, NULL);
	    msgbox->show();
		event->accept();
		if (NULL != viewer)
		{
	...
	{
		 MyZCLog::Instance().WriteToLog(ZCINFO, QString("spice channel close link "));
		 if (NULL != viewer)
		 {
			delete viewer; // $$$
			 viewer = NULL;
	...
	case SPICE_CHANNEL_ERROR_AUTH:
	{
		 ...
		 MyZCLog::Instance().WriteToLog(ZCINFO, QString("spice channel error auth "));
		if (NULL != msgbox)
		{
			delete msgbox;
			 msgbox = NULL;
...
{
	MyZCLog::Instance().WriteToLog(ZCINFO, QString(data));
	int ret = false;
	// 1. 创建 QJsonParseError 对象，用来获取解析结果
	QJsonParseError error;
	// 2. 使用静态函数获取 QJsonDocument 对象
...
void CMainWindow::netcheckfunc()
{
	...
	if (i == 1)
	{
		...
		if (viewer)
		{
			if (netdicwidget->getState() == 1)//只有这种情况下重连
			{
				netdicwidget->whide();
				viewer->CloseSpice();
				sleep(2);
